...
MyApplet.prototype = {
    ...
    _init: function(orientation, panel_height, instance_id) {
        ...
        this.settings.bind("search-filesystem", "searchFilesystem", null);
        this.refreshing = false; // used as a flag to know if we're currently refreshing (so we don't do it more than once concurrently)

        for (let i = 0; i < 20; i++) {
            this.settings.bind("quicklauncher-" + i + "-checkbox", "quicklauncher_" + i + "_checkbox", this._updateQuickLinks);
            this.settings.bind("quicklauncher-" + i + "-label", "quicklauncher_" + i + "_label", this._updateQuickLinks);
            this.settings.bind("quicklauncher-" + i + "-icon", "quicklauncher_" + i + "_icon", this._updateQuickLinks);
            this.settings.bind("quicklauncher-" + i + "-command", "quicklauncher_" + i + "_command", this._updateQuickLinks);
        }

        this.settings.bind("quicklauncher-layout", "quicklauncherLayout", this._updateQuickLinks);
        this.settings.bind("show-user-icon-label", "showUserIconLabel", this._updateQuickLinks);
